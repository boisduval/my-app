  ...
  <div>
    ...
    >
      ...
      <vxe-table-column
        ...
        fixed="left"
      ></vxe-table-column>
      <vxe-table-column field="Name" title="控制器名称" sortable width="200">
      </vxe-table-column>
      <vxe-table-column field="IDS" title="设备编码" sortable width="200">
      </vxe-table-column>
      <vxe-table-column
        field="HVersions"
        title="硬件版本号"
      ...
      <vxe-table-column field="WriteTime" title="写入时间" sortable width="200">
      </vxe-table-column>
    </vxe-table>
    <el-pagination
...
export default {
  data () {
    return {
      tableData: [],
      api: '/api/Devices/GetDevicesBmsInfo',
      list: [],
      tablePage: {
        currentPage: 1,
        // pageSize: 10,
  ...
  methods: {
    getData () {
      ...
      this.$axios
        .get(
          `${this.api}?AutoSystemID=${this.paramsD.AutoSystemID}&SystemID=${this.paramsD.SystemID}&page=${this.tablePage.currentPage}&limit=${this.tablePage.limit}`
        )
        .then(res => {
          if (res.data.code === 0) {
